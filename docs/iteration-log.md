...
## [2026-08-28] v0.2.4 修复：浅色主题下鼠标指针不清晰

### 问题
- 浅色主题下拖拽时鼠标指针显示不清晰。
- 根因：拖拽跟手浮条 .drag-ghost 带 backdrop-filter: blur(12px) 且背景接近白色半透明，
  浮条跟随指针移动时盖在指针附近，模糊层使指针（浅色背景下）发虚。

### 修复
- 移除 .drag-ghost 的 backdrop-filter（跟手提示无需模糊，去掉后指针清晰）。
- 浮条偏移从 +12/+12 增大到 +16/+22，避免遮挡指针。
- 构建产物确认 drag-ghost 无 backdrop-filter。
