...
foreach ($f in @("dsh-launcher.exe", "WebView2Loader.dll")) {
    if (-not (Test-Path (Join-Path $srcRelease $f))) {
        Write-Host "缺少 $f，请检查 tauri build 产物" -ForegroundColor Red
        exit 1
    }
}
$stage = Join-Path $env:TEMP "dsh-launcher-stage"
if (Test-Path $stage) { Remove-Item $stage -Recurse -Force }
New-Item -ItemType Directory -Force $stage | Out-Null
Copy-Item (Join-Path $srcRelease "dsh-launcher.exe") $stage -Force
...
$zip = Join-Path $releaseDir "dsh-launcher-$Version-x64.zip"
...
$setupOut = Join-Path $releaseDir "dsh-launcher-$Version-x64-setup.exe"
$nsi = Join-Path $stage "installer.nsi"
@"
Unicode true
!define APP_NAME "dsh Launcher"
!define VERSION "$Version"
Name "`${APP_NAME}"
OutFile "$setupOut"
InstallDir "`$LOCALAPPDATA\dsh-launcher"
...
Section "Install"
  SetOutPath "`$INSTDIR"
  File "dsh-launcher.exe"
  File "WebView2Loader.dll"
  File /r "resources"

  CreateDirectory "`$SMPROGRAMS\`${APP_NAME}"
  CreateShortCut "`$SMPROGRAMS\`${APP_NAME}\`${APP_NAME}.lnk" "`$INSTDIR\dsh-launcher.exe"
  CreateShortCut "`$DESKTOP\`${APP_NAME}.lnk" "`$INSTDIR\dsh-launcher.exe"

  WriteUninstaller "`$INSTDIR\uninstall.exe"
SectionEnd

Section "Uninstall"
  Delete "`$INSTDIR\dsh-launcher.exe"
...
$hashes = @()
foreach ($f in (Get-ChildItem $releaseDir -File | Where-Object { $_.Name -like "dsh-launcher-*" })) {
